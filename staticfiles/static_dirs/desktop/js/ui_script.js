...
var $AppForm = {
	ajaxSubmit: function(e, $form) {
		...
		if (fileFields.exists()) {
			...
			var cb = $AppRequest.handler({
				complete: (status, json) => {
					if (status) {
						var upload_ids = json.data['upload_ids'];
						var reqData = $form.serializeArray();
						reqData.push({ 'name': 'upload_ids', 'value': upload_ids});
						console.log(reqData);
						$AppRequest.post(action, reqData, (status, json) => {
							e.status = status;
							e.resp = json;
							handler.after(e);
						});
			...
			$.each(fileFields, function (file) {
				var files = this.files;
				var reqData = new FormData($form[0]);
				for (var index = 0; index < files.length; ++index) {
					reqData.append(this.name, files[index]);
				}
				$AppRequest.file('/upload/image/', reqData, cb);
			});
		} else {
...
var $AppNoti = {
	...
	info: function(p) {
		...
		$elm.fadeIn({duration: 1000,
			start: function() {
				$elm.find('.wt-notititle').html(opts.title);
				$elm.find('.wt-notibody').html(opts.text);
				$elm.find('.wt-notilink').attr('href', opts.link);
			}
