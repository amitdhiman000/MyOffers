$(document).ready(function() {
	...
	$(document).on('keyup', '#search-input', doSearch);
	...
	$(document).off('click', '.user-comment-submit');
 	$(document).on('click', '.user-reaction', userReactionPost);
	$(document).on('click', '.user-comment-link', userCommentsShow);
	$(document).on('click', '.user-comment-submit', userCommentPost);
});
...
function showSearchList(pThis, status)
{
	...
	if (status === true) {
		pThis.parents("#site-search-div").find("#search-results").show();
	} else {
		pThis.parents("#site-search-div").find("#search-results").hide();
	}
}
...
function doSearch(e)
{
	...
	var This = $(this);
	var listCont = This.parents("#site-search-div").find("#search-results");
...
function loadComments(pThis, artId, start, count)
{
	console.log('+loadComments');
	var data = {'article_id':artId, 'comment_start':start, 'comment_count':count};
...
function appendComment(pThis, cmt)
{
	...
	var html = '<div class ="user-comment" comment-id="'+cmt.id+'" >'
		+ '<div class="user-name" ><strong><a href="#">' + cmt.author_name + '</a></strong></div>'
